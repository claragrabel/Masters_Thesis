...
```{r}
res_test_waterLevel <- results(dds_test, contrast=c("waterLevel", "low", "high"))

summary(res_test_waterLevel, alpha=0.05)
...
```{r}
res_test_region<-results(dds_test, contrast=c("region", "Southern_Spain", "Central_Spain"))

summary(res_test_region, alpha=0.05)
```
...
```{r}
res_test_interaction <- results(dds_test, name="waterLevellow.regionSouthern_Spain", alpha = 0.05)

summary(res_test_interaction)
```
