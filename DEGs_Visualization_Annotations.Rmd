...
sig_degs_set %>%
  ...
  #   plot.margin = unit(c(1, 1, 1, 1), "cm") # Adjust the margins to create space for labels
  # ) +
  # annotate("text", x = c(-1, 1), y = c(2.5, 2.5), label = c("central", "south"), size = 4)
...
genes_to_label <- c("mmp11.1", "ass1", "sec14l3", "stard8", "leprotl1", "klf9", "hsp90")
extra_gene <- "arginase 2 [Source:Xenbase;Acc:XB-GENE-482624]"
...
gg_south <- south_data %>%
  ...
  geom_text_repel(
    data = subset(south_data, xenx_description %in% extra_gene),
    aes(label = xenp_gene_symbol),
    size = 3,
    box.padding = unit(2, "lines"),
    point.padding = unit(2, "lines"),
