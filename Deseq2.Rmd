...
dds is a  S4 object system, with slots that can be accessed through @ instead of $ (for S3 objects).
...
According to the results, there would be 52 genes which are up-regulated in low water level versus high water level, and 7 which are down-regulated.


# DOUBT
Why such small amounts of DEGs in the water and interaction comparisons independent of each other and the interaction? And not even with logfc>1 or 2, but 0. Is the region influence
filtering step too restrictive? Because I don't think tis the correction for the pvalue (BH is not very restrictive as far as I kwow)
...
# DOUBT
The numbers vary a lot even between replicates. Is this normal? or this is why we cannot take this genes into account, because it's not reliable.
...
There seem to be 2154 genes which are up-regulated, and 2420 genes which are down-regulated.
...
Are the corrections for the p-value very restrictive? BH is not very restrictive as far as I know. Or is the interaction effect very specific and only affects a small subset of genes?
...
summary(res_hBui_lBui, alpha=0.5)
```
...
summary(res_hTur_lTur, alpha=0.5)
```
...
summary(res_hEsp_lEsp, alpha=0.5)
```
...
summary(res_hLla_lLla, alpha=0.5)
```
...
summary(res_hJab_lJab, alpha=0.5)
```
...
If the question was about fixing variability within each group, why does it take into account all genes from all samples?
...
* The correction model is working, since the black dots (initial estimates) are further away to the red line than the blue dots (adjusted estimates).
* Genes with low counts show a higher dispersion, hence the curve. However, this curves flattens in genes with higher counts.
